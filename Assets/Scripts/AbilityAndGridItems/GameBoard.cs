...
public class GameBoard : MonoBehaviour
{
    ...
    public void UpdateMap(Map m)
    {
        ...
        for (int i = 0; i < m.boxSpawn.Length; i++)
        {
            GameObject obj = boxes.GetObject();
            ReplacePosition(m.boxSpawn[i].x, m.boxSpawn[i].y, obj.GetComponent<GridItem>());
        }
    }
